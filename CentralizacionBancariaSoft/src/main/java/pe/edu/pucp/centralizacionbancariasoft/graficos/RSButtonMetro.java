...
package pe.edu.pucp.centralizacionbancariasoft.graficos;
...
public class RSButtonMetro extends JButton implements MouseListener, MouseMotionListener {
    ...
    private boolean presionado = false; // Variable para indicar si el botón está presionado o no
...
//        this.setBorderPainted(false);
    ...
    public void mousePressed(MouseEvent e) {
        ...
        // Restablecer el estado de los otros botones en el contenedor
        Container parent = this.getParent();
        if (parent != null) {
            for (Component c : parent.getComponents()) {
                if (c instanceof RSButtonMetro && c != this) {
                    ((RSButtonMetro) c).setBackground(((RSButtonMetro) c).getColorNormal());
                    ((RSButtonMetro) c).setForeground(((RSButtonMetro) c).getColorTextNormal());
                    ((RSButtonMetro) c).setPresionado(false);
                }
            }
        }
    }
